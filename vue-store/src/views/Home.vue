...
export default {
  ...
  created() {
    // 获取轮播图数据
    this.$axios
      .post(this.$target + "resources/carousel", {})
      .then((res) => {
    ...
    this.getProductList(["3", "4"], "applianceList", this.$target + "good/getHotProduct");
    this.getPromo(["5", "6"], "wearingShowingList", this.$target + "good/getHotProduct");
  },
  methods: {
    // 获取家电模块子组件传过来的数据
    getChildMsg(val) {
    ...
    getPromo(categoryId, val, api) {
      api = api != undefined ? api : this.$target + "good/getProduct";
      this.$axios
        .post(api, {
